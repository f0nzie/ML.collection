...
train_data_m <-  train_data[, l]

sapply(train_data_m, function(x) is.factor(x))
```
...
# train the model
set.seed(27)
model <- train(outcome ~ ., data = train_data_m, method = "rf", preProcess = NULL, trControl = control)
...
```{r}
(l <- sapply(val_train_data, function(x) is.factor(x)))
val_train_data_m <- val_train_data[, l]

sapply(val_train_data_m, function(x) is.factor(x))
```
...
```{r echo = TRUE, message = FALSE, warning = FALSE, cache=FALSE, fig.width = 10, fig.height = 5, fig.align = "center"}
results_combined <- merge(results[, -c(1:16)], fluH7N9_china_2013[which(fluH7N9_china_2013$case_id %in% rownames(results)), ], 
...
results_combined_gather$group_dates <- factor(results_combined_gather$group_dates, levels = c("date_of_onset", "date_of_hospitalisation"))

results_combined_gather$group_dates <- mapvalues(results_combined_gather$group_dates, from = c("date_of_onset", "date_of_hospitalisation"), 
